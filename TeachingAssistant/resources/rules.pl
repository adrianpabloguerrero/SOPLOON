...
constant_in_invocation(ID):-
    (
        boolean_literal(LITERAL,parent(INVOCATION),value(VAL),body_declaration(ID),_,compilation_unit(UNIT));
        character_literal(LITERAL,parent(INVOCATION),value(VAL),body_declaration(ID),_,compilation_unit(UNIT));
        number_literal(LITERAL,parent(INVOCATION),value(VAL),body_declaration(ID),_,compilation_unit(UNIT));
        string_literal(LITERAL,parent(INVOCATION),value(VAL),body_declaration(ID),_,compilation_unit(UNIT))
    ),
    ...
    not(constant_in_invocation_checked(ID)),
    assert(constant_in_invocation_checked(ID)),
    model_unit(UNIT).

/* Constante en asignación */

constant_in_assignment(ID):-
    (
        character_literal(LITERAL,parent(PARENT),value(VAL),body_declaration(ID),_,compilation_unit(UNIT));
        number_literal(LITERAL,parent(PARENT),value(VAL),body_declaration(ID),_,compilation_unit(UNIT));
        string_literal(LITERAL,parent(PARENT),value(VAL),body_declaration(ID),_,compilation_unit(UNIT))
    ),
    not(valid_literal(VALUE)),
    (
        variable_declaration(PARENT, _, _, _, _, _, initializer(ID), _, _, _);
        assignment(PARENT, _, _, _, right_operand(ID), _, _, _)
    ),
    not(constant_in_assignment_checked(ID)),
    assert(constant_in_assignment_checked(ID)),
    model_unit(UNIT).

/* Constantes en comparaciones */

constant_in_comparision(ID):-
    (
        boolean_literal(LITERAL,parent(PARENT),value(VAL),body_declaration(ID),_,compilation_unit(UNIT));
        character_literal(LITERAL,parent(PARENT),value(VAL),body_declaration(ID),_,compilation_unit(UNIT));
        number_literal(LITERAL,parent(PARENT),value(VAL),body_declaration(ID),_,compilation_unit(UNIT));
        string_literal(LITERAL,parent(PARENT),value(VAL),body_declaration(ID),_,compilation_unit(UNIT))
    ),
    ...
    not(constant_in_comparision_checked(ID)),
    assert(constant_in_comparision_checked(ID)),
    model_unit(UNIT).

/* Operacion aritmetica con constantes */

constant_in_operation(ID):-
    (
        boolean_literal(LITERAL,parent(PARENT),value(VAL),body_declaration(ID),_,compilation_unit(UNIT));
        character_literal(LITERAL,parent(PARENT),value(VAL),body_declaration(ID),_,compilation_unit(UNIT));
        number_literal(LITERAL,parent(PARENT),value(VAL),body_declaration(ID),_,compilation_unit(UNIT))
    ...
    not(constant_in_operation_checked(ID)),
    assert(constant_in_operation_checked(ID)),
    model_unit(UNIT).

/* Constante en return */

constant_in_return(ID):-
    ...
    not(valid_literal(VALUE)),
    not(constant_in_return_checked(ID)),
    assert(constant_in_return_checked(ID)),
    model_unit(UNIT).

/* Operaciones con strings */

constant_string_in_operation(ID):-
    string_literal(LITERAL,parent(PARENT),value(VALUE),body_declaration(ID),_,_),
    not(valid_literal(VALUE)),
    infix_expression(PARENT, _, operator(O), _, _, _, _, _, compilation_unit(UNIT)),
    not(is_tostring(ID)),
    arithmetic_operator(O),
    not(constant_string_in_operation_checked(ID)),
    assert(constant_string_in_operation_checked(ID)),
    model_unit(UNIT).
