...
let nameClasses = error => {
  ...
  error.codeLocation.forEach((item, i) => {
    ...
    let aux = item.path
      .substring(item.path.lastIndexOf("/") + 1, item.path.length)
      .substring();
...
export default function TableErrors({ errors, setErrorsSelected }) {
  ...
  const handleReviewedChange = (e, error) => {
    ...
    let url =
      "https://si.isistan.unicen.edu.ar/soploon/api/users/" +
      path.userId +
      "/projects/" +
      path.projectId +
      "/corrections/" +
      ...
      actions={[
        {
          ...
          onClick: (e, data) => handleViewError(data)
        }
      ]}
      title="Errores"
      columns={[
        ...
        { title: "Proyecto", field: "nameProject" },
        {
          title: "Clase",
          field: "nameclass",
          render: error => nameClasses(error)
        },
        {
          title: "Estado",
          field: "reviewed",
          render: error => (
            <Select
              ...
              name="reviewed"
              id="select"
              onChange={event => handleReviewedChange(event, error)}
              value={error.reviewed}
            >
