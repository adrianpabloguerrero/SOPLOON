...
export default function Explorer() {
	...
	const getCorrection = (correction) => {
		...
		let url = "http://localhost:8080/soploon/api/users/" + userId + "/projects/" + projectId + "/corrections/" + date;
		
		const source = Axios.CancelToken.source();
			
		Axios.get(url, {cancelToken: source.token})
			.then(response => {
				setState({user: state.user, project: state.project, correction: response.data});
				setRequest({ source: null, error: null });
			})
			.catch(error => {
			  if (Axios.isCancel(error)) {
