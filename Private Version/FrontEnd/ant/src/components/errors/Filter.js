...
const dateToDefault = () => {
  return moment(new Date()).utcOffset("GMT-03:00").format("YYYY-MM-DD");
};
const dateFromDefault = () => {
  return moment(dateToDefault()).subtract(1, "months").format("YYYY-MM-DD");
};
const getUsers = data => {
  ...
  search = () => {
      ...
      this.dataTableFilter.forEach((user, i) => {
        if (user.selected)
          this.errors.forEach((error, i) => {
            if (error.userId === user.id) data.push(error);
          });
      });
    }
    ...
    if (this.state.searchFiltrado === "estado") {
      this.dataTableFilter.forEach((estado, i) => {
        if (estado.selected)
          this.errors.forEach((error, i) => {
            if (error.reviewed === estado.id) data.push(error);
          });
      });
    }
        ...
        {this.state.searchFiltrado !== "" && (
          <div>
            <TableFilter
              ...
              listItems={this.dataTableFilter}
