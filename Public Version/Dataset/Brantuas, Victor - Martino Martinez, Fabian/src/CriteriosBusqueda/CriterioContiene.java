...
public class CriterioContiene extends CriterioAbs {
    
    private Object valor;
    private String atributo;

    public CriterioContiene(String atributo, Object value){
        super();
        this.valor = value;
    ...
    public Vector<Pista> GetResultado(Vector<PistaAbs> LibreriaCompleta){
        ...
        for (PistaAbs p : LibreriaCompleta){
            if (p.CantElementos() == 1 && (p.getValorAtributo("id") != null)){
                if (p.getValorAtributo(atributo).toString().toLowerCase().contains(valor.toString().toLowerCase()) && (!Resultado.contains(p)))
